...
async function loadDashboard() {
  ...
  tbody.innerHTML = regs.map(r => {
    ...
    return '<tr>' +
      ...
      '<td><button class="btn btn-sm" data-region="' + esc(r.region) + '" onclick="startCollectForRegion(this.dataset.region)">Collect</button> ' +
      '<button class="btn btn-sm btn-primary" data-region="' + esc(r.region) + '" onclick="moderateRegion(this.dataset.region)">Moderate</button></td>' +
      '</tr>';
  }).join('');
...
function renderModGrid() {
  ...
  grid.innerHTML = moderationPhotos.map((p, i) => {
    ...
    return '<div class="photo-card' + (isSelected ? ' selected' : '') + '" id="card-' + p.id + '">' +
      '<input type="checkbox" class="checkbox" ' + (isSelected ? 'checked' : '') + ' data-id="' + p.id + '" onchange="toggleSelect(this.dataset.id)">' +
      '<img src="' + esc(p.cdn_url) + '" data-url="' + esc(p.cdn_url) + '" onclick="openLightbox(this.dataset.url)" loading="lazy" onerror="this.style.background=\'#1a1d27\';this.alt=\'Error\'">' +
      '<div class="info">' +
      '<div class="desc">' + esc(p.ai_description || '—') + '</div>' +
      '<div class="meta">' +
      '<span class="score-badge ' + scoreClass + '">' + (p.ai_score || '?') + '</span>' +
      '<select class="cat-select" data-id="' + p.id + '" onchange="changeCategory(this.dataset.id, this.value)">' +
      ...
      '</div>' +
      '<div class="actions">' +
      (!p.approved ? '<button class="btn btn-success btn-sm" data-id="' + p.id + '" onclick="approveOne(this.dataset.id)">Approve</button>' : '') +
      '<button class="btn btn-danger btn-sm" data-id="' + p.id + '" data-idx="' + i + '" onclick="deleteOne(this.dataset.id, parseInt(this.dataset.idx))">Delete</button>' +
      '</div></div></div>';
  }).join('');
}
